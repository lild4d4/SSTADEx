...
def check_python_version():
    ...
    if sys.version_info < (3, 10):
        print(
            "This script requires Python 3.10 or higher. Please upgrade your Python installation."
        )
        sys.exit(1)
    ...
    for requirement in requirements:
        ...
        try:
            ...
            if required_version and installed_version != required_version:
                raise ImportError(
                    f"Version mismatch for {package_name}: Expected {required_version}, found {installed_version}. Consider running 'pip install -r {requirements_file}'"
                )
        except metadata.PackageNotFoundError:
            raise ImportError(
                f"Requirement {package_name} not installed. Consider running 'pip install -r {requirements_file}'"
            )
    ...
    for tool in tool_names:
        try:
            ...
            raise EnvironmentError(
                f"{tool} not installed or not in PATH. Please install it."
            ) from e
...
def check_miniconda3_and_pdk():
    ...
    try:
        ...
        raise EnvironmentError(
            "Miniconda3 not installed or not in PATH. Please install it."
        ) from e
    ...
    missing_dirs = [
        pdk_dir for pdk_dir in required_pdk_dirs if not (pdk_root / pdk_dir).exists()
    ]
