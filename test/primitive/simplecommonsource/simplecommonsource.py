...
example.py
==========
Tests the folder-based primitive system with simplecommonsource / IHP SG13G2.

Expected output:
    - Library registers simplecommonsource from its folder
...
lib.register(ROOT / "analoglib/primitives/simplecommonsource")

# alternatively, register all primitives in the folder at once:
# lib.register_all("analoglib/primitives/")

print(lib)
# Expected: <Library 'ihp_sg13g2': ['simplecommonsource']>

# ---------------------------------------------------------------------------
# 2. Instantiate primitive and run LUT sweep
# ---------------------------------------------------------------------------
dp = lib.get("simplecommonsource", il=10e-3)
print(dp)
# Expected: <Primitive 'simplecommonsource' v1.0 | lut=path/to/LUTs/IHP_LUT_nmos.npy>
